...
def openPickledFiles():
    ...
    return trainingStates, posState, negState, wallStateMatrix
...
def trainingPath(breadcrumb):
    ...
    for paths in breadcrumb:
        for state in paths:
            x_state, y_state = state
            x = y_state * 100
            y = x_state * 100
            tempPath.append((x,y))
        gamePaths.append(tempPath)
...
def changeGame():
    ...
    trainingStates, posState, negState, wallStateMatrix  = openPickledFiles()
    ...
    trainPaths = trainingPath(trainingStates)
    wallRectCoord = gamePath(wallStateMatrix)
    
    return trainingStates, posState, negState, wallStateMatrix, trail, availableState, trainPaths, wallRectCoord

trainingStates, posState, negState, wallStateMatrix, trail, availableState, gamePaths, wallRectCoord = changeGame()

print(trainingStates[:2], posState, negState, wallStateMatrix)
#print(trail)
#print(availableState)
#print('gamePaths: ', gamePaths[0])
#print('wallRectCoord: ', wallRectCoord)
...
def changeGame():
    ...
    return game_path, trail, path, angleList, expandedGamePath, expandedAngle, wallRectCoord#, wallStateMatrix
'''
# --------------------pygame----------------------------------------
def playGame(gamePath, wallRectCoord):
    # initialize
    pygame.init()
    ...
    #buildingImage = pygame.image.load('images/build_1.png')
    ...
    def game_loop(gamePath, wallRectCoord):
        ...
        while not gameExit:
            ...
            for rect in wallRectCoord:
                #pygame.draw.rect(gameDisplay, black, Rect((rect[0], rect[1]), (100, 100)))
    ...
    game_loop(gamePath, wallRectCoord)
...
if __name__ == '__main__':
    ...
    #game_path, trail, path, angleList, expandedGamePath, expandedAngle, wallRectCoord = changeGame()
    #playGame(expandedGamePath, expandedAngle, wallRectCoord)

    trainingStates, posState, negState, wallStateMatrix, trail, availableState, gamePaths, wallRectCoord = changeGame()
    playGame(trainingStates, wallRectCoord)
'''
