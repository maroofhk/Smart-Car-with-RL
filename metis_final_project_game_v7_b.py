...
def breadcrumb(policeState, q_matrix, row_max, col_max, num_rows, num_cols, posState, negState):
    '''
...
def changeGame(policeState):
    ...
    trail, path = breadcrumb(policeState, q_matrix, row_max, col_max, num_rows, num_cols, posState, negState)
    ...
    # intermediate angles
    moveMatrix = makeMoveMatrix(q_matrix, num_rows, num_cols, row_max, col_max, posState, negState )
    ...
    return game_path, trail, expandedGamePath
 
def playGame(expandedPath, policeState):
    pygame.init()
    ...
    def game_loop(expandedPath, policeState):
        ...
        while not gameExit:
            ...
            if pressed[pygame.K_UP] and (policeState[0]*100>= 100):
                ...
                _, _, expandedPath = changeGame(policeState)
                counter = 0
                footPathList = []
            ...
            for element in footPathList:
                showPoliceCar(element[0]+30, element[1]+30)

            if counter < len(expandedPath):
    ...
    game_loop(expandedPath, policeState)
...
if __name__ == '__main__':
    ...
    policeState = (6,0)
    game_path, trail, expandedGamePath = changeGame(policeState)
    playGame(expandedGamePath, policeState)